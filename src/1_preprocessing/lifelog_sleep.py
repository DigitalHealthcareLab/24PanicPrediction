...
sleep['date'] = sleep['Date'].str[:10] 
sleep['Date'] = pd.to_datetime(sleep['Date'], format = '%Y-%m-%d %H:%M:%S')
sleep['Bedtime'] = pd.to_datetime(sleep['Bedtime'], format = '%Y-%m-%d %H:%M:%S')
sleep['Wakeup_time'] = pd.to_datetime(sleep['Wakeup_time'], format = '%Y-%m-%d %H:%M:%S')
sleep['sleep_duration'] = sleep['Wakeup_time'] - sleep['Bedtime'] 
sleep['sleep_in'] = sleep['Date'] - sleep['Bedtime'] 
sleep['sleep_out'] = sleep['Date'] - sleep['Wakeup_time'] 
...
sleep.drop(['Date','Bedtime','Wakeup_time'], axis=1, inplace=True)
